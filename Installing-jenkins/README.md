...
- Follow `Manage Jenkins` -> `Manage Plugins` path and install the plugins (install without restart):
  ...
  -  `Locale`   # Bununla dili degistirebiliriz.
...
- Create a public project repository `Jenkins_hands_on` on your GitHub account.

- Clone the `Jenkins_hands_on` repository on local computer.

```bash
git clone https://github.com/latifyildirim/Jenkins_hands_on.git
```
- Create a `Jenkinsfile` within your local `Jenkins_hands_on` repo and save following pipeline script. Consider that filename 'Jenkinsfile' is case sensitive.
...
```text
https://github.com/latifyildirim/Jenkins_hands_on
```
